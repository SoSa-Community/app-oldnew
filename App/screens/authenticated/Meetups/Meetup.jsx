	...
	if (Array.isArray(comments) && comments.length) {
		return comments.map((item, index) => (
			<CommentItem
				comment={item}
				key={item.id}
				onFacePress={() => showMemberProfile(item?.user?.user_id)}
			/>
		));
	}
};
		...
		return (
			<View style={Styles.attendeesContainer}>
				{attendees.map((attendee) => (
					<View
						style={Styles.attendeeImageContainer}
						key={attendee?.id}>
						<Image
...
const MeetupImage = ({ meetup, children }) => {
	let imageSource = {};
	if (meetup?.image) {
		imageSource = { uri: meetup.image };
	} else {
		imageSource = require('../../../assets/choose_meetup_image_v2.jpg');
	}
...
const MeetupScreen = ({ navigation, route, showMemberProfile }) => {
	...
	const {
		services: { meetups: meetupService, comments: commentsService },
	} = useAPI();
		...
		meetupService
			...
			.then((meetup) => {
				setAttendees([
					{
						id: 1,
						picture: `https://botface.io/generate?seed=${Math.random()}`,
					},
					{
						id: 2,
						picture: `https://botface.io/generate?seed=${Math.random()}`,
	...
	const sendComment = () => {
		...
		setSending(true);
		commentsService
			.create('sosa', comment, 'meetup', meetup?.id)
			.then((comment) => {
	...
	const buildWrapper = (children) => {
		...
		return (
			<KeyboardAvoidingView
				...
				behavior="padding"
				keyboardVerticalOffset={Math.floor(
					(dimensions.height / 100) * 9,
				)}>
				{children}
			</KeyboardAvoidingView>
		);
	...
	return buildWrapper(
		<View style={{ flex: 1, backgroundColor: '#121111' }}>
			...
			<View style={Styles.buttonContainer}>
				...
				<View style={{ flex: 1 }}>
					<AttendingButton
						{...{ meetup, attendees, toggleGoing, saving }}
					/>
				</View>
			</View>
