...
const CreateMeetupScreen = ({navigation}) => {
    ...
    const { services: { meetups: meetupService, general: generalService } } = useAPI();
    const { modals } = useApp();
    ...
    const [ description, setDescription ] = useState('This is a test description to make sure it works');
    const [ type, setType ] = useState('virtual');
    const [ uploading, setUploading ] = useState(false);
    const [ isValid, setIsValid ] = useState({title: false, date:true, description: false, type: true});
    const [ errors, setErrors ] = useState({});
    
    
    
    useEffect(() => {
        setMenuOptions({showLeft:true, showRight: false, leftMode: 'back', title: 'Create Meetup'})
    }, []);
    ...
    const resetErrors = () => setErrors({name:'', date:'', description: '', type:''});
    
    const save = () => {
        resetErrors();
        setSaving(true);

        meetupService.create('sosa',
            title,
            description,
            type,
    ...
    const buttons = () => {
        const resetButton = image ? <ActivityButton text="Reset" style={{backgroundColor: 'rgba(125, 125, 200, 0.70)', paddingHorizontal: 24, borderRadius: 16, marginRight: 8}} onPress={() => setImage('')} /> : null;
        if(!uploading) {
            return (
    ...
    return (
            <View style={{flex:1, backgroundColor: '#121111'}}>
                    ...
                    <ScrollView style={{flex:1}} scrollEnabled={true} contentContainerStyle={{ flexGrow: 1 }}>
                        ...
                        <View style={{flex:1, paddingHorizontal:4, marginTop: 8}}>
                            <Input
                                ...
                                minLength={16}
                                maxLength={250}
                                setIsValid={(isValid) => updateIsValid('title', isValid)}
                            />

                            <Input label="When is it?" icon={['fal', 'calendar-week']} error={errors?.date} placeholder="Date" type="date" value={date} onChangeText={(data, date) => setDate(data)} />
                            <View style={{flexDirection: 'row', marginVertical:4}}>
                                <View style={{flex:1}}>
                                    <Input icon={['fal', 'clock']} error={errors?.date} errorBorderOnly placeholder="When does it start?" type="time" value={start} onChangeText={(data, date) => setStart(data)} />
                                </View>
                                <Text style={{flex:0, color: '#fff', fontSize:24, textAlignVertical:'center', marginHorizontal: 8}}>-</Text>
                                <View style={{flex:1}}>
                                    <Input icon={['fal', 'clock']} error={errors?.date} errorBorderOnly placeholder="When does it end?" type="time" value={end} onChangeText={data => setEnd(data)} />
                                </View>
                            </View>
                            ...
                            <Input
                                ...
                                minLength={16}
                                maxLength={0}
                                setIsValid={(isValid) => updateIsValid('description', isValid)}
                ...
                <View style={Styles.buttonContainer}>
                    <View style={{flex:1}}>
                        <ActivityButton text="Create" style={{}} onPress={save} showActivity={saving} disabled={!checkIsValid()}/>
                    </View>
                </View>
            </View>
