...
const ChatScreen = ({navigation, showMemberProfile}) => {
    ...
    const { update: updateDrawerItem, closeDrawers, addHeaderIcon, openRightDrawer } = useAuthenticatedNavigation();
    ...
    const [ newMessagesNotificationVisible, setNewMessagesNotificationVisible ] = useState(false);
    const [ slowDownNotifierVisible, setSlowDownNotifierVisible ] = useState(false);
    ...
    const setMessageInput = (data) => {
        ...
        if(Platform.OS === 'ios') checkForTags();
    }
    ...
    const setupChat = () => {
        if(!settingUp){
            ...
            updateRoomList().finally(() => {
                if(currentRoom) joinRoom(currentRoom?.community_id, currentRoom?.name);
                settingUp = false
            });
        }
    }
	...
	const preferencesChanged = (newPreferences) => {
		...
		if(updateState) setPreferences(stateChanges);
	}
	...
	const sendMessage = (messageToSend) => {
		...
		if(!coolDown && slowDownCounter < 3){
			...
			if(uploadedImages.length) {
			    message = `${message} ${uploadedImages.map(({uri}) => uri).join(' ')}`;
            }
			...
			if(message.length > 0){
				...
				coolDownTimer = setTimeout(() => coolDown = false, slowDownCounter * 200);
				...
				chatService.rooms.send(community_id, name, message)
	...
	const renderRoomList = (updatedRoomList) => {
		if(!updatedRoomList){ updatedRoomList = [...rooms]; }

		let roomViews = updatedRoomList.map((room) => {
			return <RoomItem key={room.id}
					     onPress={() => {
						     if(!currentRoom || currentRoom?.name !== room.name) joinRoom(community, room.name)
	...
	const updateRoomList = () => {
        return chatService.rooms.list(community)
            .then((rooms) => {
                setRooms(rooms);
                return rooms;
		    })
    ...
    const joinRoom = (communityID, roomID) => {
        ...
        chatService.rooms.join(communityID, roomID)
    ...
    const checkForTags = () => {
		...
		if(atIndex !== -1){
			...
			if(space === -1) space = messageInput.length;

			if(space >= end){
				let part = messageInput.substring(atIndex + 1, space).trim().toLowerCase();
        ...
        middleware.add('chat', {
            ...
            'api_event': ( packet ) => {
                ...
                return new Promise((resolve, reject) => {
                    if(type === 'chat/message') return addMessage(data);
    ...
    const renderImageList = () => {
        const images = uploadedImages.map((imageData, index) => {
            if(imageData){
                ...
                return (<TouchableOpacity key={index} style={{margin:2, borderRadius: defaultSize / 2, overflow:'hidden'}} onPress={() => {uri && setPreviewEmbed(imageData)}} onLongPress={() => {uri && deleteImage(index)}} >
        ...
        if(currentRoom) {
            const displayUserList = () => {
                if(currentRoom !== null){
                    ...
                    modals?.create('You\'re not in a room','Please join a room first!');
                }
            };
                ...
                <View style={Styles.messageListContainer}>
                    <MessageList
                         ...
                         onMessageAdded={(isScrolled, item) => {
                            setNewMessagesNotificationVisible(isScrolled && !newMessagesNotificationVisible);
                         }}
                         {...{onFacePress, onLongFacePress, addTag, uploadedImages, setUploadedImages, setPreviewEmbed, preferences}}
                    />
                    ...
                    
                    {
                        ...
                        <TouchableHighlight onPress={messageList?.current?.scrollToBottom} style={Styles.newMessageScrollNotifier}>
                    ...
                    
                    {
                        ...
                        <TouchableHighlight onPress={() => setSlowDownNotifierVisible(false)} style={Styles.slowDownNotifier}>
                ...
                <View style={Styles.footer}>
                    <MessageInput
                        ...
                        onSelectionChange={(event) => {
                            ...
                            if(Platform.OS === 'android') checkForTags();
                        }}
                        fuckWith={fuckWith}
                        canSend={canSend}
                        uploading={uploading}
