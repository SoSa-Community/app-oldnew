	...
	add(namespace = '', event, middleware, signature = '') {
		...
		let add = (event, middleware, signature) => {
			if (!signature || signature.length === 0)
				signature = this.client.generateUUID();

			if (!this.middleware[namespace]) this.middleware[namespace] = {};
			if (!this.middleware[namespace][event])
				this.middleware[namespace][event] = {};
	...
	trigger(event, data) {
		return new Promise((resolve, reject) => {
			if (event) {
				let callbacks = [];
				console.debug(
					'Client::Middleware::trigger::namespace',
					event,
					this.middleware,
				);
						...
						callbacks = [
							...
							...Object.values(
								...
								new Promise((resolve1, reject1) => {
									...
									console.debug(
										'Client::Middleware::trigger::error',
										event,
										error,
									);
								}),
