...
export class GeneralProvider extends RequestProvider {
	...
	connect = () => {
		return new Promise((resolve, reject) => {
			...
			if (socket && socket.connected) {
				console.info(
					'Client::GeneralProvider::connect::already_connected',
				);
				resolve();
			} else {
				if (!this.connecting) {
					...
					console.info(
						'Client::GeneralProvider::connect',
						this.config,
					);
					...
					listeners.add({
						...
						connect: (socket) => {
							...
							console.info(
								'Client::GeneralProvider::connect::connected',
							);
							...
							return middleware
								...
								.then((data) => {
									console.info(
										'Client::GeneralProvider::connect::authenticated',
									);
								...
								.catch((error) => {
									console.info(
										'Client::GeneralProvider::connect::authentication_failed',
										error,
									);
									return middleware.trigger(
						...
						connect_timeout: (error) => {
							...
							return middleware
								.trigger('connection_timeout', error)
								.then((error) => {
									console.debug('Connect timeout: ', error);
								});
						},
	...
	authenticate(reauthAttempted) {
		...
		return this.getAuthEntities(true, true).then(
			({ device, session, jwt }) => {
				console.info(
					'Client::GeneralProvider::authenticate',
					device,
					session,
					jwt,
				);
				...
				if (!device || (!session && !isBot) || !jwt)
					throw new SoSaError('invalid_session');
				...
				return new Request(this, '', 'authenticate', payload)
					...
					.catch((error) => {
						console.info(
							'Client::GeneralProvider::authenticate::failed',
							error,
						);

						if (
							!reauthAttempted &&
							typeof sessionHandler === 'function'
						) {
							sessionHandler
								.reauth()
