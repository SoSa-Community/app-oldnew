...
export class ChatService {
	...
	rooms = {
		 ...
		 */
		list: (communityID) => {
			return new Request(this.provider, 'rooms', 'list', {
				community_id: communityID,
			})
				.run()
				.then(({ data: { rooms } }) =>
					rooms.map((room) => Room.fromJSON(this.client, room)),
				);
		},
		...
		join: (communityID, roomID, password = '', room = null) => {
			...
			return new Request(this.provider, 'rooms', 'join', data)
				...
				.then(({ data }) => {
					...
					let parsedHistory = history.map((message) =>
		...
		leave: (communityID, roomID) => {
			return new Request(this.provider, 'rooms', 'leave', {
				community_id: communityID,
				room_id: roomID,
			}).run();
		},
		...
		online: (communityID, roomID) => {
			return new Request(this.provider, 'rooms', 'online', {
				community_id: communityID,
				room_id: roomID,
			})
				.run()
		...
		send: (communityID = '', roomID = '', message = '', uuid = '') => {
			...
			return new Request(this.provider, 'rooms', 'message', {
				community_id: communityID,
				room_id: roomID,
				message: message,
				uuid: uuid,
