...
const Modals = forwardRef((props, ref) => {
	...
	return modals.map((modal, index) => {
		...
		return (
			<Modal visible={true} transparent={true} key={index}>
...
const AppProvider = ({ children, ...props }) => {
	...
	const triggerMiddleware = (event, data) => {
		return new Promise((resolve, reject) => {
			if (event) {
				let callbacks = [];
				console.debug(
					'App::Middleware::trigger::namespace',
					event,
					middleware,
				);
				...
				for (const namespace in middleware) {
					if (middleware[namespace].hasOwnProperty(event)) {
						callbacks = [
							...callbacks,
							...Object.values(middleware[namespace][event]).map(
								(middleware) =>
	...
	const addMiddleware = (
		...
		let add = (event, callback) => {
			if (!middleware[event] || onlyOneAllowed) {
				middleware[event] = [];
			}
			...
			if (!middleware[namespace][event]) {
				middleware[namespace][event] = [];
			}
	...
	Linking.getInitialURL().then((result) =>
		handleDeepLink({ url: result }, true),
	);
	...
	useEffect(() => {
		...
		Device.init().then((device) => {
			...
			return Session.init().then((session) => {
	...
	return (
		<AppContext.Provider
			value={{
				...
				modals: {
					create: (title, description, onClose) =>
						modals?.current?.create(title, description, onClose),
				},
				appInitialized,
			}}
			{...props}>
