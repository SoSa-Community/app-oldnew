...
const AuthProvider = (props) => {
	...
	const logout = (sessionAutoExpired) => {
		...
		if (sessionAutoExpired === true) {
			...
			Alert.alert(
				'Oooof',
				'Sorry you were logged out, please login again!',
					...
					{ text: 'Cancel', style: 'cancel' },
					{
						...
						onPress: () => {
							...
							authService
								.logout()
								.catch((error) =>
									console.debug('App::logout', error),
								);
						},
					},
				],
	...
	const login = (username, password) => {
		return new Promise((resolve, reject) => {
			if (!username) {
				reject(new Error('provide_username'));
			} else if (!password) {
				reject(new Error('provide_password'));
			} else {
				authService
	...
	const register = (username, password, email) => {
		return new Promise((resolve, reject) => {
			if (!username) {
				...
				reject(new Error('provide_email'));
			} else {
				authService
	...
	const socialLogin = (forLogin, network) => {
		return authService.createPreauth().then((id) => {
			preauthID = id;
			Linking.openURL(
				authService.getPreauthURI(
